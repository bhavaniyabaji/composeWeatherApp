...
import com.user.weatherapp.data.model.ForecastResponse.Current.Condition
import com.user.weatherapp.model.Forecast
import com.user.weatherapp.model.Hour
...
import com.user.weatherapp.ui.weather.components.ForecastDayComponent
import com.user.weatherapp.ui.weather.components.WeatherComponent
import com.user.weatherapp.utils.DateUtil.toFormattedDate
...
private fun WeatherSuccessState(
    ...
    Column(
        ...
        Text(
            modifier = Modifier.padding(top = 12.dp).testTag("test_city"),
            text = uiState.weather?.name.orEmpty(),
            style = MaterialTheme.typography.headlineMedium
        )
        Text(
            modifier = Modifier.testTag("test_date"),
            text = uiState.weather?.date?.toFormattedDate().orEmpty(),
            style = MaterialTheme.typography.bodyLarge
        )

        AsyncImage(
            modifier = Modifier.size(64.dp).testTag("test_condition"),
        ...
        uiState.weather?.forecasts?.let { forecasts ->
            ForecastDayComponent(modifier= Modifier.fillMaxWidth(),forecasts)
        }
